...
import gzip
import os
def simulate_true_prs(suffix, causal_inds,m,h2,n_admix,prefix="output/sim1/",
    ...
    """
    print("Load simulation trees")
    tree_CEU = msprime.load(prefix+path_tree_CEU)
    tree_YRI = msprime.load(prefix+path_tree_YRI)
    print("Compute random effects for true PRS causal variants")
    var_dict = _compute_effects(m, h2,causal_inds)
    print("Calculate true PRS for Europeans and Africans")
    prs_CEU = calc_prs_tree(var_dict,tree_CEU)
    prs_YRI = calc_prs_tree(var_dict,tree_YRI)
    print("Calculate true PRS for admixed individuals")
    prs_admix,samples_admix = calc_prs_vcf(prefix+vcf_file,var_dict,n_admix)
    print("Combine all true PRS results")
    prs_comb = np.concatenate((prs_CEU,prs_YRI,prs_admix),axis=None)
    print("Write true PRS as hdf")
    prs_norm, sample_ids = _write_true_prs(suffix, tree_CEU,tree_YRI,m,h2,
                                prs_comb,np.array(list(var_dict.values())),
                                prefix,samples_admix)
    print("Get training and testing samples for creating and testing")
    # the GWAS estimated PRS
    _split_case_control_all_pops(suffix,m,h2,prs_norm, sample_ids, n_admix,prefix)
    print("For each admixed individual calculate the global proportion ")
...
def _compute_effects(m, h2,causal_inds):
    """
...
def _write_true_prs(suffix,tree_ceu,tree_yri,m,h2,prs,effects,prefix,sample_ids_admix):
    """
    Write PRS to file
    ...
    with h5py.File(prefix+"true_prs/prs_m_{}_h2_{}_{}.hdf5".format(m,h2, suffix), 'w') as f:
...
def _split_case_control_all_pops(suffix,m,h2,G,labels,N_ADMIX,prefix):
    """
    ...
    # write data as hdf
    with h5py.File(prefix+'true_prs/prs_m_{}_h2_{}_{}.hdf5'.format(m,h2, suffix), 'a') as f:
